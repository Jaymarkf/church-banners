...
export default class Cart extends PageManager {
    onReady() {
        ...
        //custom
        //for adding customer also brought in cart page
        // const productIds = [123, 1234];  Replace with your product IDs or dynamically ids
        const productIds = Array.from(document.querySelectorAll('[cartproductid]'))
                        .map(el => parseInt(el.getAttribute('cartproductid'), 10));
        const params = { template: 'products/single-product' }; // Optional parameters
        this.getMultipleProducts(productIds, params)
        .then(products => {
          if(!(products.length == 0 || products.every(item => item === ''))){
          $('ul.productGrid').append(products); // Handle the retrieved products
          // You can update the UI or perform any other actions with the products here  
          $('ul.productGrid').slick({
            slidesToShow: 4,
            slidesToScroll: 1,
            dots: false,
             responsive: [
                {
                    breakpoint: 1268, // when window width is <= 1268px
                    settings: {
                        slidesToShow: 3
                    }
                },
                {
                    breakpoint: 1000, // when window width is <= 1268px
                    settings: {
                        slidesToShow: 2
                    }
                }
            ...
            $('.container-col-left h4').remove();
            $('.container-col-left ul.productGrid').remove();
            $('.container-contact-actions').css({"border-top": "none"});
          }

        // Loader was visible initially, now remove it after success
        $('ul.productGrid').removeClass('loaderz');
    ...
    getMultipleProducts(productIds, params) {
    const promises = productIds.map(productId => {
        return new Promise((resolve, reject) => {
        utils.api.product.getById(productId, params, (err, product) => {
            if (err) {
            reject(err);
            } else {
            resolve(product);
